...
#import "ConfigProgressView.h"
#import "ConfigNetworkDisconnectView.h"

@interface ConfigContentController ()<WKUIDelegate,WKNavigationDelegate,UIGestureRecognizerDelegate,ConfigControlViewDelegate>{
    WKWebView *kWKWebView;
    ConfigControlView *contentBottomView;
    AFNetworkReachabilityStatus networkStatus;
}

@property (nonatomic,strong) ConfigProgressView *progressView;
@property (nonatomic,strong) ConfigNetworkDisconnectView *networkDisconnectView;

@end

#define kProgressHeight     6.0f
@implementation ConfigContentController
...
- (void)viewDidLoad {
    ...
    contentBottomView = [[ConfigControlView alloc] initWithFrame:CGRectMake(0, ConfigHeight - ConfigTabBarHeight, ConfigWidth, ConfigTabBarHeight)];
...
- (ConfigProgressView*)progressView{
    if (_progressView == nil) {
        _progressView = [[ConfigProgressView alloc] initWithFrame:CGRectMake(-kProgressHeight, hConfigNaviBarHeight, kProgressHeight, kProgressHeight)];
...
- (ConfigNetworkDisconnectView*)networkDisconnectView{
    if (_networkDisconnectView == nil) {
        _networkDisconnectView = [[ConfigNetworkDisconnectView alloc] initWithFrame:CGRectMake(0, 0, ConfigWidth, ConfigHeight)];
        [self.view addSubview:_networkDisconnectView];
    }
    [_networkDisconnectView reloadDisconnectView];
...
- (void)performOperationWithStyle:(ConfigControlStyle)style{
    switch (style) {
        ...
        case ConfigControlMenu:
        {
            ...
            [controller addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {[self openContentUseB];}]];
            [self presentViewController:controller animated:YES completion:nil];
        }
            break;
        case ConfigControlHome:
        {
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    if ([self isAppDomain:webView.URL.absoluteString]) {
        ...
        [kWKWebView evaluateJavaScript:@"document.documentElement.style.webkitTouchCallout='none';" completionHandler:^(id _Nullable obj, NSError * _Nullable error) {}];
        [kWKWebView evaluateJavaScript:@"document.documentElement.style.webkitUserSelect='none';" completionHandler:^(id _Nullable obj, NSError * _Nullable error) {}];
    }
}
